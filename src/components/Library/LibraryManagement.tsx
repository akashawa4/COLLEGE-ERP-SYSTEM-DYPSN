            ...
            <div key={member.id} className="bg-white border rounded-lg p-3 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-500 mt-1">{member.email} • {member.phone}</div>
                </div>
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <div className="text-sm text-gray-700">{member.currentBooksIssued}/{member.maxBooksAllowed} books</div>
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      setEditingMember(member);
          ...
          {filteredTransactions.map(tx => (
            <div key={tx.id} className="bg-white border rounded-lg p-3 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  <div className="text-sm font-medium text-gray-900">{tx.bookTitle}</div>
                  <div className="text-xs text-gray-500">{tx.memberName}</div>
                  <div className="text-xs text-gray-500 mt-1">Due: {new Date(tx.dueDate).toLocaleDateString()}</div>
                </div>
                <div className="text-right">
                  <div className={`px-2 py-1 rounded-full text-xs ${
                    tx.status === 'Issued' ? 'bg-blue-100 text-blue-800' :
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="text-sm text-gray-700">Fine: ₹{tx.fineAmount}</div>
                <div className="flex gap-2">
                  {tx.status === 'Issued' && (
                    <button
                      onClick={() => {
                        if (confirm(`Return book "${tx.bookTitle}"?`)) {
                          libraryTransactionService.returnBook(tx.id, user?.name || 'Library Staff').then(() => loadData());
                        }
                      }}
                      className="px-3 py-2 bg-green-600 text-white rounded"
                    >Return</button>
    ...
    <div className="max-w-7xl mx-auto p-4 lg:p-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 mb-6">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4 sm:gap-8 px-4 sm:px-6 overflow-x-auto">
            {[
              ...
              <button
                ...
                className={`flex-none flex items-center gap-2 py-3 sm:py-4 px-2 sm:px-4 border-b-2 font-medium text-sm ${
                  activeTab === tab.id
